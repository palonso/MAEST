...
def update_state_dict_names(state_dict):
    replacements = [
        ...
        ("patch_embed.proj.", "audio_spectrogram_transformer.embeddings.patch_embeddings.projection."),
    ...
    for k, v in state_dict.items():
        if "qkv" in k:
            ...
            for mat, name in zip([qu, ke, va], ["query", "key", "value"]):
                new_values[f"audio_spectrogram_transformer.encoder.layer.{layer}.attention.attention.{name}.{type}"] = mat
...
def recombine_pos_embeddings(state_dict):
    ...
    state_dict["audio_spectrogram_transformer.embeddings.position_embeddings"] = pos_embs
...
for model in models:
    ...
    configuration = ASTConfig(
        ...
        time_stride=10,
        id2label={k: normalize("NFKC", v) for k, v in enumerate(discogs_labels)},
        label2id={normalize("NFKC", v): k for k, v in enumerate(discogs_labels)},
        layer_norm_eps=1e-6,
        torch_dtype="float16",
    )

...
def exp_rampdown(rampdown_length, num_epochs):
    ...
    def warpper(epoch):
        if epoch >= (num_epochs - rampdown_length):
            ep = 0.5 * (epoch - (num_epochs - rampdown_length))
...
def cosine_rampdown(rampdown_length, num_epochs):
    ...
    def warpper(epoch):
        if epoch >= (num_epochs - rampdown_length):
            ep = 0.5 * (epoch - (num_epochs - rampdown_length))
            return float(0.5 * (np.cos(np.pi * ep / rampdown_length) + 1))
        else:
            return 1.0
...
def cosine_cycle(cycle_len=20, ramp_down_start=100, last_lr_value=0.01):
    ...
    def warpper(epoch):
        ep = (epoch + cycle_len // 2.0) / (1.0 * cycle_len)
        if epoch > ramp_down_start:
            return last_lr_value
        return float(
            last_lr_value + (1.0 - last_lr_value) * 0.5 * (np.cos(2.0 * np.pi * ep) + 1)
        )
